...
- `E [all INTCMP E']` evaluates to true when `e INTCMP E'` is true for every element `e` in `E`.
- `E [any INTCMP E']` evaluates to true when `e INTCMP E'` is true for some element `e` in `E`.

But what happens if evaluating `e INTCMP E'` errors on some element `e`? In this case, the evaluation of `all` or `any` terminates abruptly with a distinguished `QuantifierError`.
