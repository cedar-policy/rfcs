...
However, there are some sharp corners in the current design of unspecified entities (described in detail below), so this RFC proposes a few changes:

1. This RFC proposes to instead refer to these special entities as **default entities** since you should use them when you want _some default value_ to pass into an authorization request, but you don't care what the exact value is because it doesn't impact authorization.
2. This RFC proposes to introduce a new special type (`__cedar::Default`) and request entity variant (`Default`) that allow users to refer to default entities explicitly.

Under this proposal, the schema definition for the "createFile" action above would look like:
...
We expect that this concept will only become more confusing once we stabilize the partial evaluation experimental feature, which allows “unknown” entities that are subtly different from "unspecified" entities, despite sharing similar names (see below).
...
So a request for the example above might look like:
...
We have found this _implicit_ way of referring to unspecified entities (from this point on called "default entities") to be confusing, so this RFC proposes to introduce a new special type (`__cedar::Default`) that allows users to refer to default entities _explicitly_.

This requires changing the type of the `Request` constructor from:
...
Furthermore, schemas will need to explicitly say that an action applies to the default type, rather than omitting an `appliesTo` field.
...
To start, we will leave the current APIs and schema format as-is, while also adding the new `__cedar::Default` type and support for using it in schemas.
...
Then, in a subsequent major version, we will implement the breaking changes described above.

### Documentation

As pointed out in reviews of this RFC, a key reason why unspecified/default entities are confusing is that we have almost no documentation for them.
Thus, an important part of this RFC is developing useful documentation.
We expect this to include a blog post with examples of applications where you may want to use a default entity, and recommendations for best practices.
